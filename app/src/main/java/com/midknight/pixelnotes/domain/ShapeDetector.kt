    ...
    if (straightDist < max(w, h) * 0.35f && w > 30f && h > 30f) {
        ...
        if (edges in 3..5) {
            if (edges == 4) {
                ...
                if (isAxisAligned) {
                    return listOf(
                        PointData(minX, minY, 1f), PointData(maxX, minY, 1f),
                        PointData(maxX, maxY, 1f), PointData(minX, maxY, 1f),
                        PointData(minX, minY, 1f)
                    )
                }
            }
            return simplified
            ...
            for (i in 0..36) {
                ...
                ovalPoints.add(
                    PointData(
                        (cx + a * cos(angle)).toFloat(),
                        (cy + b * sin(angle)).toFloat(),
                        1f
                    )
                )
            }
